class Course:
    ...
    def doGrading(self,students):
        temp=list(students.printer().items())
        temp.sort(key=lambda x:x[1][-1],reverse=True)
        policy_calc=dict.fromkeys(self.policy,0)
        ...
        for i in range(len(temp)):
            if track<=3:
                if temp[i][1][-1]<self.policy[track]+2 and temp[i][1][-1]>self.policy[track]-2:
                    policy_calc[self.policy[track]].append(temp[i][1][-1])
                elif temp[i][1][-1]<self.policy[track]-2:
                    track+=1
        for i in range(len(policy_calc)):
            if len(policy_calc[key[i]])==1:
                for x in range(len(self.policy)):
                    if self.policy[x]==key[i]:
                        self.policy[x]=policy_calc[key[i]]
            else:
                ...
                for x in range(len(self.policy)):
                    if self.policy[x]==key[i]:
                        self.policy[x]=round(sum(start)/2,2)

    def return_policy(self):
        return self.policy

    def summary(self,students):
        sumo={"A":0,"B":0,"C":0,"D":0,"F":0}
        temp=list(students.printer().items())
        for i in range(len(temp)):
            sumo[temp[i][1][-1]]+=1
        return [self.cname,self.credits,self.assessments,self.policy,sumo]

class Student:
    def __init__(self):
        self.students={}
    
    def add(self,roll,marks):
        if roll not in self.students:
            self.students[roll]=marks
            self.students[roll].append(round(sum(self.students[roll]),1))
            return 0
        else:
            return -1
    ...
    def grade(self,policy):
        key=list(self.students.keys())
        for i in range(len(self.students)):
            if self.students[key[i]][-1]>=policy[0]:
                self.students[key[i]].append("A")
            elif self.students[key[i]][-1]>=policy[1]:
                self.students[key[i]].append("B")
            elif self.students[key[i]][-1]>=policy[2]:
                self.students[key[i]].append("C")
            elif self.students[key[i]][-1]>=policy[3]:
                self.students[key[i]].append("D")
            else:
                self.students[key[i]].append("F")
    
    def print_all(self):
        temp=self.students.copy()
        key=list(self.students.keys())
        ans=[]
        for i in range(len(self.students)):
...
for i in range(len(total)):
    ...
    log.add(total[i][0],total[i][1:])

ip=Course(cname,credits,assessments,policy)
ip.doGrading(log)
log.grade(ip.return_policy())

def test():
    assert ip.summary(log)==['IP', 4, [('labs', 30), ('midsem', 15), ('assignments', 30), ('endsem', 25)], [80.9, 65.9, 51.0, 41.2], {'A': 19, 'B': 136, 'C': 305, 'D': 245, 'F': 294}]
    assert log.finder("2024492")==[28.7, 9.4, 22.9, 14.1, 75.1, 'B']
    assert log.finder("2024999")==[10.7, 14.0, 27.3, 0.1, 52.1, 'C']
    assert log.finder("2024001")==[23.1, 2.2, 28.6, 1.2, 55.1, 'C']
